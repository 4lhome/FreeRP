...
namespace FreeRP.Test.Server
{
    ...
    public class TestFrpDataService
    {
        ...
        public GrpcService.Content.ContentUriRequest TestFileFile { get; set; } = new() { Uri = $"file://{nameof(TestFileFile)}" };
        ...
        public TestFrpDataService()
        {
            ...
            _fileData = new byte[1024 * 1024];
            Array.Fill<byte>(_fileData, 23);
        }
        ...
        public async Task TestAll()
        {
            ...
            DataAccessFile.Change = DataAccessFile.Create = DataAccessFile.Delete = DataAccessFile.Read = false;

            await FileCreateAsync();
            await FilePathChange();
            await FileOpenAsync();
            await FileDeleteAsync();

            await DatabaseCreateAsync();
            await DatabaseChangeAsync();
            await DatabaseDeleteAsync();
        ...
        public async Task FileCreateAsync()
        {
            var res = await _frpDataService.FileCreateAsync(TestFileFile, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorAccessDenied);

            DataAccessFile.Create = true;
            await DataAccessChangeAsync();
            res = await _frpDataService.FileCreateAsync(TestFileFile, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorNone);

            for (int i = 0; i < 10; i++)
            {
                res.Data = Google.Protobuf.UnsafeByteOperations.UnsafeWrap(_fileData);
                res = await _frpDataService.FileStreamWriteAsync(res);
                Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorNone);
            }
            ...
            res = await _frpDataService.FileStreamWriteAsync(res);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorNone);
            Assert.IsTrue(_frpDataService.UpDict.IsEmpty);
        }

        public async Task FilePathChange()
        {
            GrpcService.Content.ChangeContentUriRequest change = new() { 
                SourceUri = TestFileFile.Uri,
                DestUri = TestFileFile.Uri + 2,
            };

            var res = await _frpDataService.FilePathChangeAsync(change, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorAccessDenied);

            DataAccessFile.Change = true;
            await DataAccessChangeAsync();
            res = await _frpDataService.FilePathChangeAsync(change, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorNone);
            TestFileFile.Uri = change.DestUri;
        }

        public async Task FileOpenAsync()
        {
            var res = await _frpDataService.FileOpenAsync(TestFileFile, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorAccessDenied);

            DataAccessFile.Read = true;
            await DataAccessChangeAsync();
            res = await _frpDataService.FileOpenAsync(TestFileFile, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorNone);

            MemoryStream ms = new();
            while (res.EOF == false) 
            {
                res = await _frpDataService.FileStreamReadAsync(res);
                Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorNone);
                await ms.WriteAsync(res.Data.Memory);
            }
            Assert.IsTrue(ms.Length == 11 * 1024 * 1024);
            ms.Dispose();
        }

        public async Task FileDeleteAsync()
        {
            var res = await _frpDataService.FileDeleteAsync(TestFileFile, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorAccessDenied);

            DataAccessFile.Delete = true;
            await DataAccessChangeAsync();
            res = await _frpDataService.FileDeleteAsync(TestFileFile, _authService);
            Assert.IsTrue(res.ErrorType == GrpcService.Core.ErrorType.ErrorNone);
        }
        ...
        public async Task DatabaseCreateAsync()
        {
            var res = await _frpDataService.DatabaseAdd(new(), _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorDatabaseId);

            res = await _frpDataService.DatabaseAdd(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorNone);
            Assert.IsTrue(Database.Change == false && Database.Create == false && Database.Delete == false && Database.Read == false);

            res = await _frpDataService.DatabaseAdd(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorExist);

            var db = _frpDataService.DatabaseGet(Database.DatabaseId);
            Assert.IsNotNull(db);
        }

        public async Task DatabaseChangeAsync()
        {
            GrpcService.Database.DatabaseTable dt = new();
            dt.Fields.Add(new GrpcService.Database.DatabaseTableField() 
            { 
                DataType = GrpcService.Database.DatabaseDataType.FieldString
            });
            Database.Tables.Add(dt);

            var res = await _frpDataService.DatabaseChangeAsync(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorDatabaseTableId);

            dt.TableId = "t1";
            res = await _frpDataService.DatabaseChangeAsync(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorDatabaseFieldIdNotExist);

            dt.Fields[0].IsId = true;
            res = await _frpDataService.DatabaseChangeAsync(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorDatabaseFieldId);

            dt.Fields[0].FieldId = "f";
            res = await _frpDataService.DatabaseChangeAsync(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorNone);
        }

        public async Task DatabaseDeleteAsync()
        {
            var res = await _frpDataService.DatabaseDeleteAsync(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorNone);

            res = await _frpDataService.DatabaseDeleteAsync(Database, _frpSettings.AdminId);
            Assert.IsTrue(res == GrpcService.Core.ErrorType.ErrorNotExist);
        }
